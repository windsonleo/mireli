...
InserirPaciente($ativo,$nome,$datanascimento,$foto,$email,$telefone,$psicologo,$conn);


 function InserirPaciente($ativo,$nome,$datan,$foto,$email,$telefone,$psicologo,$conexao){
  $conn = $conexao;

$sql = "INSERT INTO paciente (id, ativo, nome, datanascimento, foto, email, telefone,psicologo_id,dataregistro)
VALUES (NULL, '$ativo', '$nome', '$datan', '$foto' , '$email', '$telefone','$psicologo', CURRENT_TIMESTAMP)";

if (mysqli_query($conn, $sql)) {
  echo "Registro paciente incluido com sucesso";
  mysqli_close($conn);
   header("Location: http://agenda.com/pacientess.php");
   exit();
...
if (mysqli_query($conn, $sql)) {
  echo "Registros listados paciente  com sucesso";
